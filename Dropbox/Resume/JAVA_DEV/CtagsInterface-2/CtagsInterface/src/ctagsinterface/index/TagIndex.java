...
public class TagIndex
{
	...
	public TagIndex() throws RuntimeException
	{
		...
		fieldAnalysers.put(_NAME_FLD, keywordAnalyzer);
		fieldAnalysers.put(_NAME_LOWERCASE_FLD, keywordAnalyzer);
		fieldAnalysers.put(_PATH_FLD, keywordAnalyzer);
		fieldAnalysers.put(_ORIGIN_FLD, keywordAnalyzer);
		// Origin documents
		fieldAnalysers.put(ORIGIN_ID_FLD, keywordAnalyzer);
		fieldAnalysers.put(TYPE_FLD, keywordAnalyzer);
		analyzer = new PerFieldAnalyzerWrapper(standardAnalyzer, fieldAnalysers);
	...
	private static File getIndexPath() throws RuntimeException
	{
		File pluginHome = EditPlugin.getPluginHome(CtagsInterfacePlugin.class);
		if (pluginHome == null)
		...
		if (!index.isDirectory() && !index.mkdirs())
			throw new RuntimeException("CtagsInterface plugin cannot work without a settings directory.");
		return index;
	}
	...
	public String getOriginsOfFile(String file)
	{
		...
		q.add(getTermQuery(DOCTYPE_FLD, TAG_DOC_TYPE), BooleanClause.Occur.MUST);
		q.add(getPathQuery(file), BooleanClause.Occur.MUST);
		final StringBuilder sb = new StringBuilder();
		runQuery(q, 1, new DocHandler()
		{
			public void handle(Document doc)
			{
	...
	{
		BooleanQuery q = new BooleanQuery();
		q.add(getTermQuery(_NAME_FLD, tag.getName()), BooleanClause.Occur.MUST);
		q.add(getPathQuery(tag.getFile()), BooleanClause.Occur.MUST);
		q.add(getTermQuery(PATTERN_FLD, tag.getPattern()), BooleanClause.Occur.MUST); 
		deleteQuery(q);
	}
		...
		if (extensions != null)
		{
			for (String s: extensions)
			{
				if (! s.equals(LINE_FLD))
					q.add(getTermQuery(s, tag.getExtension(s)), BooleanClause.Occur.MUST);
			}
		}
	 ...
	 * If a tag belongs to multiple origins, only remove the specified origin from it.
	 */
	public void deleteTagsOfOriginAndFilePath(Logger logger, final Origin origin,
		String filePath)
	{
		...
		String filePathStr = "";
		if(filePath!=null) {
			if (origin.type.equals(OriginType.MISC))
		...
		boolean createIfNotExists)
	{
		...
		if (! b[0])
		{
			...
			doc.add(new StringField(DOCTYPE_FLD, ORIGIN_DOC_TYPE, Store.YES));
			doc.add(new StringField(TYPE_FLD, type.name, Store.YES));
			doc.add(new StringField(ORIGIN_ID_FLD, id, Store.YES));
			try { writer.addDocument(doc); }
			catch (IOException e) { e.printStackTrace(); }
			endActivity();
		}
	...
	static public TermQuery getTermQuery(String field, String value)
	{
		return getAlreadyEscapedTermQuery(field, escape(value));
	}

	static public TermQuery getAlreadyEscapedTermQuery(String field, String value)
	{
		return new TermQuery(new Term(field, value));
	}

	static public TermQuery getPathQuery(String file)
	{
		return getTermQuery(_PATH_FLD, file);
	}
	...
	public void queryTags(Query query, int maxResults, final List<Tag> tags)
	{
		if (tags == null)
			return;
	...
	{
		Document doc = new Document();
		doc.add(new TextField(_NAME_FLD, t.getName(), Store.YES));
		//todo: Matthieu : do not store
		doc.add(new StringField(_NAME_LOWERCASE_FLD, t.getName().toLowerCase(), Store.YES));
		doc.add(new Field(PATTERN_FLD, t.getPattern(), Store.YES, Index.ANALYZED));
		doc.add(new StringField(PATH_FLD, t.getFile(), Store.NO));
		doc.add(new TextField(_PATH_FLD, t.getFile(), Store.YES));
		...
		doc.add(new StringField(DOCTYPE_FLD, TAG_DOC_TYPE, Store.YES));
		return doc;
	}
	...
	{
		doc.add(new StringField(ORIGIN_FLD, originsStr, Store.YES));
		doc.add(new TextField(_ORIGIN_FLD, originsStr, Store.NO));
	}
		...
		int maxResults, final List<Tag> tags)
	{
		...
		{
			if (origin.type.equals(OriginType.MISC)) {
				q.add(getPathQuery(origin.id.toString()), BooleanClause.Occur.SHOULD);
			} else {
				String escaped = escape(origin.toString());
				q.add(getAlreadyEscapedTermQuery(_ORIGIN_FLD, "*" + escaped),
					BooleanClause.Occur.SHOULD);
				q.add(getAlreadyEscapedTermQuery(
					_ORIGIN_FLD, "*" + escaped + Origin.SEP + "*"),
						BooleanClause.Occur.SHOULD);
			}
		}
	...
	public void runQuery(String query, int maxResults, DocHandler handler)
	{
		...
		if (q == null)
			return;
		runQuery(q, maxResults, handler);
	}

	private void runQuery(Query q, int maxResults, DocHandler handler)
	{
