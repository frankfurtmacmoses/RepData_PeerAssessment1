...
import ctagsinterface.dockables.Preview;
...
public class CtagsInterfacePlugin extends EditPlugin
{
	...
	private static final String PREVIEW = "ctags-interface-preview";
	...
	/** Adds a source directory tree.
	  * @param path The directory path, may contain a trailing slash or not.
	  */
	static public void addTree(View view, String path)
	{
		if (path.endsWith(File.separator))
			path = path.substring(0, path.length() - 1);
		Logger logger = CtagsInterfacePlugin.getLogger(view,
			OriginType.DIRECTORY.name + " " + path);
		insertOrigin(logger, OriginType.DIRECTORY, path);
	}
	...
	/** Context menu action from FSB, adds selected items. 
	  * If no FSB item is selected, the current directory is added as
	  * a source tree. */
	public static void addFromFSB(View view, VFSBrowser browser)
	{
	    VFSFile[] vfsfiles = browser.getSelectedFiles();

		if (vfsfiles.length == 0)
		{
		    addTree(view, browser.getDirectory());
		    return;
		}

		for (VFSFile f: vfsfiles)
		{
			if (f.getType() == VFSFile.DIRECTORY)
			{
				addTree(view, f.getPath());
			}
			else if (f.getType() == VFSFile.FILE)
			{
				addFile(view, f.getPath());
			}
			// other VFSFile types are ignored - we are happy with
	...
	// Action: Previews the selected tag (or tag at caret).
	public static void previewTag(final View view)
	{
		DockableWindowManager dwm = view.getDockableWindowManager();
		dwm.showDockableWindow(PREVIEW);
		Preview preview = (Preview) dwm.getDockable(PREVIEW);
		preview.previewTag();
	}
	...
	// Show a query dialog
	public static void tagProjectAction(final View view)
	{
		if (pvi == null)
		{
			JOptionPane.showMessageDialog(view, "Project support disabled.");
		...
		if (project != null)
			refreshOrigin(OriginType.PROJECT, project);
	}
	...
	public static Vector<Tag> queryFileTags(String file, int maxResults)
	{
		Vector<Tag> tags = new Vector<Tag>();
		index.queryTags(TagIndex.getPathQuery(file), maxResults, tags);
		return tags;
	}
