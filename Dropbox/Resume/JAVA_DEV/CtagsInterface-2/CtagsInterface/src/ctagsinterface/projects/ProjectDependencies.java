...
public class ProjectDependencies extends AbstractOptionPane
{
	...
	private static final String FILE_DEPENDENCY = "fileDependency";
	JList projects;
	JList trees;
	JList tagFiles;
	JList files;
	...
	protected void _init()
	{
		...
		addSeparator();
		filesModel = getListModel(FILE_DEPENDENCY);
		files = createList("Files:", filesModel, new DependencyAsker () {
			public String getDependency() {
	...
	private void refreshDependencies()
	{
		...
		refreshDependency(FILE_DEPENDENCY, OriginType.MISC);
	}
	...
	private void refreshDependency(String dependencyType, OriginType originType)
	{
		...
		if (originType.equals(OriginType.MISC))
			origins = CtagsInterfacePlugin.getIndex().getFilesOfOrigin(OriginType.MISC);
	...
	{
		Vector<String> filesVec = CtagsInterfacePlugin.getIndex().getFilesOfOrigin(OriginType.MISC);
		return getSelectionDialog(filesVec, filesModel, "File");
	}
	...
	protected void _save()
	{
		...
		setListModel(FILE_DEPENDENCY, filesModel);
	}
	...
	public static HashMap<String, Vector<String>> getDependencies(String projectName)
	{
		...
		Vector<String> fileDeps = getListProperty(project, FILE_DEPENDENCY);
		map.put(TagIndex.OriginType.MISC.name, fileDeps);
		return map;
	}
